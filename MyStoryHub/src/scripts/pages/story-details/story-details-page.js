...
export default class StoryDetailsPage {
  ...
  async populateStoryDetailsAndInitialMap(message, story) {
    try {
      ...
      saveOfflineBtn.innerHTML = '<i class="fas fa-download"></i> Favorite Offline';
      ...
      if (savedStory) {
        ...
        saveOfflineBtn.innerHTML = '<i class="fas fa-check"></i> Favorite Offline';
      }
  ...
  renderSaveButton() {
    ...
    if (saveContainer) {
      ...
      document.getElementById('story-details-save')?.addEventListener('click', async () => {
        ...
        if (response) {
          alert('Story Favorite Successfully!');
        }
      });
    }
  }
